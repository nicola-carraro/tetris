...
static void ttsUpdate(TtsTetris *tetris, float secondsElapsed) {
    ...
    TtsTetramino patterns[TtsTetraminoType_Count] = {
        [TtsTetraminoType_I] = {{{0, -1}, {0, 0}, {0, 1}, {0, 2}}},
    };

    //platformDebugPrint("%llu\n", TTS_ARRAYCOUNT(tetraminos));

    if (tetris->frame == 0) {
        platformPlayMusic(tetris, tetris->music);
        tetris->playerXInCells = 5.0f;
        tetris->playerYInCells = 6.0f;
    ...
    // Player
    {
        ...
        TtsTetramino playerPattern = patterns[tetris->playerType];

        for (int32_t cellIndex = 0; cellIndex < TTS_ARRAYCOUNT(playerPattern.cells); cellIndex++) {
            TtsCellCoords cellCoords = playerPattern.cells[cellIndex];
            int32_t column = (int32_t)tetris->playerXInCells + cellCoords.x;
            int32_t row = (int32_t)tetris->playerYInCells + cellCoords.y;
